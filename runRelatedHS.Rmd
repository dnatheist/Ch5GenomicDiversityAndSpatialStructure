---
title: "Half Sibling Relatedness Between Murray Cod Larvae 2011-2013"
author: "Alan Couch"
...
read_chunk('halfSibsMatingHatch.R')
```
...
#Limit to half siblings
net.HS <- delete_edges(net, E(net)[weight>0.4])
net.HS <- delete_edges(net.HS, E(net.HS)[weight<0.15])
l <- layout_with_fr(net.HS)

plot(net.HS, edge.arrow.size=0, edge.curved=0.2, vertex.size=5,vertex.color=c("orange", "cyan")[(V(net.HS)$type=="larvae")+1], vertex.frame.color="#555555",vertex.label=V(net)$name, vertex.label.color="black",vertex.label.cex=.7,layout=l, main = main)
```


```{r Larvae2011hd, echo=TRUE,tidy=TRUE, tidy.opts=list(comment=FALSE)}
<<halfSibsHatchDiff>>
...
```{r Larvae2012hd, echo=TRUE,tidy=TRUE, tidy.opts=list(comment=FALSE)}
<<halfSibsHatchDiff>>
...
plot(net.HS, edge.arrow.size=0, edge.curved=0.2, vertex.size=5,vertex.color=c("orange", "cyan")[(V(net.HS)$type=="larvae")+1], vertex.frame.color="#555555",vertex.label=V(net)$name, vertex.label.color="black",vertex.label.cex=.7,layout=l, main = main)
```
```{r Larvae2013hd, echo=TRUE,tidy=TRUE, tidy.opts=list(comment=FALSE)}
<<halfSibsHatchDiff>>
